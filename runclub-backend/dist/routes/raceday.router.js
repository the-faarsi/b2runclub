...
router.post("/check-in", (0, auth_1.requireRole)(CREW), async (req, res) => {
    try {
        ...
        if (party.length === 1) {
            ...
            const now = new Date();
            await prisma_1.default.registrationGuest.update({
                where: { id: only.id },
 ...
 * un-admitted.
 */
async function syncBookingAttendance(registrationId, crewId) {
...
async function loadGuest(id) {
    return prisma_1.default.registrationGuest.findUnique({
        where: { id },
        include: {
            registration: {
                include: {
 ...
 */
router.post("/guests/:guestId/admit", (0, auth_1.requireRole)(CREW), async (req, res) => {
    try {
        const guest = await loadGuest(req.params.guestId);
        if (!guest) {
        ...
        const party = await prisma_1.default.registrationGuest.findMany({
...
router.post("/guests/:guestId/unadmit", (0, auth_1.requireRole)(CREW), async (req, res) => {
    try {
        const guest = await loadGuest(req.params.guestId);
        if (!guest) {
            res.status(404).json({ error: "Unrecognised person on this booking" });
...
router.post("/check-in/:registrationId/undo", (0, auth_1.requireRole)(CREW), async (req, res) => {
    try {
         ...
         */
        await prisma_1.default.registrationGuest.updateMany({
 ...
 */
function toPeople(regs) {
    return regs.flatMap((r) => {
        const rows = r.guests?.length > 0
            ? r.guests
        ...
        return rows.map((g) => ({
            ...
            user_id: g.is_booker ? r.user.id : null,
            name: g.name,
            kind: g.kind,
            is_booker: Boolean(g.is_booker),
            booked_by: r.user.name,
    ...
    try {
        ...
        res.json({
            ...
            turnout: {
                ...
                blocked: toPeople(registrations.filter((r) => r.blocked_at)).length,
