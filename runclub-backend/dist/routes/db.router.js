...
async function listTables() {
    const rows = await prisma_1.default.$queryRawUnsafe("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name");
...
router.get("/tables/:table", (0, auth_1.requireRole)(["ADMIN"]), async (req, res) => {
    try {
        ...
        const rows = await prisma_1.default.$queryRawUnsafe(`SELECT * FROM "${table}" ${where} ORDER BY "${sortCol}" ${dir} LIMIT ? OFFSET ?`, ...params, limit, offset);
        res.json({
            table,
            primary_key: pk,
...
router.patch("/tables/:table/:id", (0, auth_1.requireRole)(["ADMIN"]), async (req, res) => {
    try {
        ...
        const affected = await prisma_1.default.$executeRawUnsafe(`UPDATE "${table}" SET ${setClause} WHERE "${pk}" = ?`, ...prepared.values, req.params.id);
        if (affected === 0) {
...
router.delete("/tables/:table/:id", (0, auth_1.requireRole)(["ADMIN"]), async (req, res) => {
    try {
        ...
        const affected = await prisma_1.default.$executeRawUnsafe(`DELETE FROM "${table}" WHERE "${pk}" = ?`, req.params.id);
        if (affected === 0) {
            res.status(404).json({ error: "No row with that key" });
            return;
        }
