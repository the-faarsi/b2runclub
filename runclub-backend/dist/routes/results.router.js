...
router.get("/events/:id", async (req, res) => {
    try {
        ...
        const others = rows.filter((r) => r.status !== "FINISHED" || !r.finish_secs);
 ...
 */
router.put("/events/:id", (0, auth_1.requireRole)(["ADMIN"]), async (req, res) => {
    try {
        const eventId = req.params.id;
        ...
        const secs = finish_secs === undefined || finish_secs === null || finish_secs === ""
            ? null
        ...
        const result = await prisma_1.default.eventResult.upsert({
            ...
            create: {
                ...
                finish_secs: secs,
                distance_km: distance_km === undefined || distance_km === ""
                    ? null
...
router.delete("/:resultId", (0, auth_1.requireRole)(["ADMIN"]), async (req, res) => {
    try {
        await prisma_1.default.eventResult.delete({ where: { id: req.params.resultId } });
...
router.get("/me", (0, auth_1.requireRole)(["MEMBER", "VOLUNTEER", "ADMIN"]), async (req, res) => {
    try {
        const rows = (await prisma_1.default.eventResult.findMany({
            where: { user_id: req.user.id },
            include: { event: true },
            orderBy: { event: { date_time: "desc" } },
        }));
        const finished = rows.filter((r) => r.status === "FINISHED" && r.finish_secs);
        res.json({
            results: rows.map((r) => ({
                id: r.id,
                event_id: r.event_id,
            ...
            totals: {
                ...
                total_distance_km: Number(finished
...
router.post("/feedback/:eventId", (0, auth_1.requireRole)(["MEMBER", "VOLUNTEER", "ADMIN"]), async (req, res) => {
    try {
        ...
        const reg = await prisma_1.default.eventRegistration.findFirst({
        ...
        const feedback = await prisma_1.default.eventFeedback.upsert({
...
router.get("/feedback/me/:eventId", (0, auth_1.requireRole)(["MEMBER", "VOLUNTEER", "ADMIN"]), async (req, res) => {
    try {
        const mine = await prisma_1.default.eventFeedback.findFirst({
...
router.get("/feedback/:eventId", (0, auth_1.requireRole)(["ADMIN"]), async (req, res) => {
    try {
        const rows = (await prisma_1.default.eventFeedback.findMany({
 ...
 */
router.get("/streaks/me", (0, auth_1.requireRole)(["MEMBER", "VOLUNTEER", "ADMIN"]), async (req, res) => {
    try {
        const regs = (await prisma_1.default.eventRegistration.findMany({
        ...
        if (weeks.length) {
            ...
            if (last === thisWeek || last === thisWeek - WEEK)
                current = run;
        }
        const total = attended.length;
        const volunteered = regs.filter((r) => r.role_at_event === "VOLUNTEER").length;
        const badges = [
