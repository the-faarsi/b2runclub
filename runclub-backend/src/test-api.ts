...
async function runTests() {
    ...
    try {
        ...
        const memberRegData = (await memberReg.json()) as any;
        ...
        const volunteerRegData = (await volunteerReg.json()) as any;
        ...
        const adminToken = ((await adminLog.json()) as any).token;
        ...
        const memberToken = ((await memberLog.json()) as any).token;
        ...
        const volunteerToken = ((await volunteerLog.json()) as any).token;
        ...
        if (badEventRes.status !== 403) throw new Error("RBAC failed: Member were not forbidden");
        ...
        const draftEvent = ((await draftEventRes.json()) as any).event;
        ...
        const pubEvent = ((await pubEventRes.json()) as any).event;
        ...
        const pubList = (await pubListRes.json()) as any;
        console.log("Public visible events count:", pubList.length, "(Expected: 1)");
        if (pubList.some((e: any) => e.status !== "PUBLISHED")) throw new Error("Public got unpublished events");
        ...
        if (badReg.status !== 400) throw new Error("Allowed registration without signing waiver");
        ...
        const memberRegDetails = (await memberRegRes.json()) as any;
        ...
        const volRegDetails = (await volRegRes.json()) as any;
        ...
        const regCheck = await prisma.eventRegistration.findUnique({
        ...
        if (regCheck?.status !== "PAID") throw new Error("Webhook signature execution failed to update status to PAID");
        ...
        const notifications = (await memberNotifyRes.json()) as any;
        console.log("Member notifications count received:", notifications.length, "(Expected: 2)");
        const announceNotify = notifications.find((n: any) => n.message.includes("Hydration"));
        if (!announceNotify) throw new Error("Notification broadcast failed to reach member");
        ...
        const poll = ((await pollRes.json()) as any).poll;
        ...
        const vote2Data = (await vote2.json()) as any;
        console.log("Double vote error message:", vote2Data.error);
        if (vote2.status !== 400) throw new Error("Unique composite constraint bypassed: voted twice");
        ...
        if (overview.total_revenue !== 250.0) throw new Error("Revenue calculations are incorrect");
