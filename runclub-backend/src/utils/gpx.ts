...
export interface TrackPoint {
    ...
    lon: number;
    ele: number | null;
    /** Recorded timestamp, when the device wrote one. Used to derive duration. */
    time: Date | null;
}
...
export function parseGpx(xml: string): TrackPoint[] {
    ...
    const re =
        /<(?:trkpt|rtept)[^>]*\blat="([-\d.]+)"[^>]*\blon="([-\d.]+)"[^>]*>([\s\S]*?)<\/(?:trkpt|rtept)>|<(?:trkpt|rtept)[^>]*\blat="([-\d.]+)"[^>]*\blon="([-\d.]+)"[^>]*\/>/g;
...
export function summariseRoute(points: TrackPoint[]) {
    let distance = 0;
    let gain = 0;
...
export function timespanOf(points: TrackPoint[]) {
    const stamps = points.map((p) => p.time).filter((t): t is Date => t !== null);
