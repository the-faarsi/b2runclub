...
export function authorizeUrl(userId: string): string {
    if (!stravaConfigured) throw new Error("Strava is not configured on this server");

    const state = jwt.sign({ uid: userId, purpose: "strava-oauth" }, JWT_SECRET, {
        expiresIn: "15m",
    });
...
export function userIdFromState(state: string): string | null {
    try {
        const payload = jwt.verify(state, JWT_SECRET) as { uid?: string; purpose?: string };
        if (payload.purpose !== "strava-oauth" || !payload.uid) return null;
        return payload.uid;
    } catch {
        return null;
    }
...
async function tokenRequest(body: Record<string, string>): Promise<StravaTokens> {
    ...
    return JSON.parse(text) as StravaTokens;
}
...
export async function accessTokenFor(userId: string): Promise<string | null> {
    const account = await prisma.stravaAccount.findUnique({ where: { user_id: userId } });
...
export const finishUrl = (outcome: "connected" | "denied" | "error", detail?: string) => {
    const params = new URLSearchParams({ strava: outcome });
    if (detail) params.set("reason", detail.slice(0, 140));
