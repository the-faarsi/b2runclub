...
import { AuthRequest, requireAccount, requireRole } from "../middleware/auth";
...
router.get("/config", async (_req: AuthRequest, res: Response): Promise<void> => {
    res.json({
        ...
        missing: stravaConfigured ? [] : ["STRAVA_CLIENT_ID", "STRAVA_CLIENT_SECRET"],
    });
});
 ...
 */
router.get("/authorize", requireAccount, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!stravaConfigured) {
            res.status(503).json({
...
router.get("/callback", async (req: AuthRequest, res: Response): Promise<void> => {
    const { code, state, error: denied } = req.query as Record<string, string | undefined>;

    // The athlete pressed "Cancel" on Strava's consent screen.
    if (denied) {
        res.redirect(finishUrl("denied"));
        return;
    }

    if (!code || !state) {
        res.redirect(finishUrl("error", "Strava's response was missing the code"));
        return;
    }

    const userId = userIdFromState(state);
    if (!userId) {
        res.redirect(finishUrl("error", "That linking request expired — try again"));
        return;
    }
    ...
    try {
        ...
        if (!athlete?.id) {
            res.redirect(finishUrl("error", "Strava didn't return an athlete"));
            return;
        }
         ...
         * figures.
         */
        const clash = await prisma.stravaAccount.findUnique({ where: { athlete_id: athleteId } });
        if (clash && clash.user_id !== userId) {
            res.redirect(
                finishUrl("error", "That Strava account is already linked to another member"),
            );
            return;
        }
        ...
        await prisma.user.update({ where: { id: userId }, data: { strava_id: athleteId } });
        ...
        res.redirect(finishUrl("connected"));
    } catch (error: any) {
        console.error("[strava] callback failed:", error?.message || error);
        res.redirect(finishUrl("error", error?.message || "Token exchange failed"));
    }
});
...
router.get("/me", requireAccount, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if (!account) {
            res.json({ connected: false, configured: stravaConfigured });
            return;
        }
 ...
 */
router.get("/activities", requireAccount, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const token = await accessTokenFor(req.user!.id);
        if (!token) {
        ...
        const upstream = await fetch(`${STRAVA_API}/athlete/activities?per_page=${perPage}`, {
...
router.delete("/me", requireAccount, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const account = await prisma.stravaAccount.findUnique({ where: { user_id: req.user!.id } });
        if (!account) {
        ...
        await prisma.stravaAccount.delete({ where: { user_id: req.user!.id } });
        await prisma.user.update({ where: { id: req.user!.id }, data: { strava_id: null } });
...
router.get("/leaderboard", async (_req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        res.json({
             ...
             */
            live: false,
            source: stravaConfigured ? "sample" : "sample",
        });
    } catch (error: any) {
 ...
 */
router.post("/link", requireRole(["MEMBER", "VOLUNTEER", "ADMIN"]), async (_req: AuthRequest, res: Response): Promise<void> => {
    res.status(410).json({
