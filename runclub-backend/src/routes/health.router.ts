...
import { looksLikeHealthExport, parseAppleHealthExport, type ImportedWorkout } from "../utils/appleHealth";
import { parseGpx, summariseRoute, timespanOf } from "../utils/gpx";
...
const IMPORT_DIR = path.join(os.tmpdir(), "b2-health-imports");
...
const importUpload = multer({
    storage: multer.diskStorage({
        ...
        filename: (_req, _file, cb) =>
            cb(null, `${Date.now()}-${crypto.randomBytes(6).toString("hex")}.xml`),
    }),
    ...
    fileFilter: (_req, file, cb) => {
        ...
        if (name.endsWith(".zip")) {
            return cb(
                new Error(
                    "Unzip the export first and upload the export.xml from inside it.",
                ),
            );
        }
        if (name.endsWith(".xml") || name.endsWith(".gpx")) return cb(null, true);
...
function workoutFromGpx(xml: string, filename: string): ImportedWorkout | null {
    const points = parseGpx(xml);
    if (points.length < 2) return null;

    const summary = summariseRoute(points);
    const span = timespanOf(points);
 ...
 */
router.post("/import", requireRole(MEMBERS), (req: AuthRequest, res: Response) => {
        ...
        try {
            ...
            if (isGpx) {
                ...
                if (!workout) {
                    ...
                    res.status(400).json({
                        error:
                            "That GPX has no timestamped track points, so there's no date or duration to log.",
                    });
                    return;
                }
                workouts = [workout];
                ...
                if (!(await looksLikeHealthExport(file.path))) {
                    cleanup();
                    res.status(400).json({
                        error:
                            "That doesn't look like an Apple Health export. Look for export.xml inside the archive Health gives you.",
                    });
                    return;
                }

                const outcome = await parseAppleHealthExport(file.path);
                seen = outcome.seen;
            ...
            const existing = new Set(
                (
                    await prisma.healthWorkout.findMany({
...
router.get("/me", requireRole(MEMBERS), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const window = (days: number) => all.filter((w) => +w.started_at >= since(days));

        const totalsFor = (set: typeof all) => ({
            workouts: set.length,
        ...
        const byType = new Map<string, { count: number; distance_km: number }>();
        for (const w of all) {
...
router.delete("/:id", requireRole(MEMBERS), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const workout = await prisma.healthWorkout.findUnique({
        ...
        await prisma.healthWorkout.delete({ where: { id: workout.id } });
...
router.delete("/", requireRole(MEMBERS), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { count } = await prisma.healthWorkout.deleteMany({
            where: { user_id: req.user!.id },
        });
