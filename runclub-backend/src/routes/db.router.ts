 ...
 */
async function resolveTable(input: unknown): Promise<string | null> {
...
const toNumber = (v: unknown) => (typeof v === "bigint" ? Number(v) : v);

function redactRow(row: Record<string, unknown>) {
 ...
 */
function friendlyDbError(error: any, table: string): string {
...
router.get("/tables", requireRole(["ADMIN"]), async (_req: AuthRequest, res: Response): Promise<void> => {
    try {
        const tables = await listTables();
        const out = [];
        for (const name of tables) {
            const [{ n }] = await prisma.$queryRawUnsafe<{ n: number | bigint }[]>(
 ...
 */
router.get("/tables/:table", requireRole(["ADMIN"]), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
 ...
 */
function prepareValues(cols: ColumnInfo[], body: unknown) {
    if (!body || typeof body !== "object") return { error: "A JSON object of column values is required" };
    ...
    for (const [key, raw] of Object.entries(body as Record<string, unknown>)) {
        ...
        if (!col) return { error: `"${key}" is not a column on this table` };
        ...
        let value = raw;
        if (value === "" && !col.notnull) value = null;
        ...
        if (value !== null && value !== undefined) {
            if (col.type === "INTEGER") {
                ...
                if (!Number.isInteger(n)) return { error: `"${col.name}" must be a whole number` };
                value = n;
            } else if (col.type === "REAL") {
                const n = Number(value);
                if (!Number.isFinite(n)) return { error: `"${col.name}" must be a number` };
                value = n;
    ...
    if (names.length === 0) return { error: "No values supplied" };
    return { names, values };
}

/** 3. Insert a row. */
router.post("/tables/:table", requireRole(["ADMIN"]), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const table = await resolveTable(req.params.table);
        if (!table) {
            res.status(404).json({ error: "No such table" });
...
router.patch("/tables/:table/:id", requireRole(["ADMIN"]), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const table = await resolveTable(req.params.table);
        if (!table) {
            res.status(404).json({ error: "No such table" });
...
/** 5. Delete a row. */
router.delete("/tables/:table/:id", requireRole(["ADMIN"]), async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const table = await resolveTable(req.params.table);
        if (!table) {
            res.status(404).json({ error: "No such table" });
